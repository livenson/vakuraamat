...
|---|---|---|
| `heightmap.r32`, `canopy.r32` | the laser sheets under the tile, reprojected to EPSG:3301 | Done (step 1). A 1024 m tile on the Estonian grid touches at least four 1 km sheets (170–300 MB each; 470 MB for the Old Town, 1.2 GB for Cēsis). Downloading them side by side does not help (measured 5.4 MB/s on one connection, 5.5 MB/s in total on four), so since 2026-09-12 a new world is centred on the sheet holding the searched place (`sources.Latvia.place`, `fetch_tile_lv.place_center`). It then overhangs that sheet by about 12 m a side, and a sheet whose share of the tile is narrower than `SKIP_M` = 40 m is not downloaded (`select_sheets`): the strip is filled from the ground beside it and never counts as open water. Its canopy and roofs are 0, so a building there takes the cadastre's storeys. One sheet instead of four; the grids turn by at most 0.1° across Latvia. A flat-filled strip met the real ground of the next tile with a step (Cēsis: 0.33 m typically, 4 m on the Gauja's side; extending the slope into the strip did not help), so a tile built beside one that already stands bends its first `BLEND_M` = 40 m of ground to meet that tile's edge (`adjacent_tiles`, `blend_edges`). The searched place is kept as `terrain.focus`, and the player starts there. Ground: classes 2 and 14 averaged per metre, holes under buildings filled; cells with no return at all in areas wider than 17 m are open water, set to the median water-surface height (0.39 m on the Daugava), because interpolating from the banks streaked across the river. Canopy: the highest **vegetation** return (3–5) above the ground, so a roof never reads as a tree, unlike Maa-amet's nDSM, which the building footprints have to mask |
...
| 4 | LĢIA orthophoto cycles 1–5 (1994–99 B/W 1 m, 2003–05 1 m, 2007–08 and 2010–11 0.5 m, 2013–15 0.25–0.4 m) from the `LGIA_OpenData_Ortofoto_*_saites.txt` lists; a window is cut over `/vsicurl/` with the `.tfw` beside it (`CPL_VSIL_CURL_ALLOWED_EXTENSIONS=.tif,.tfw`) | coordinates | **done 2026-09-12**: the plot page's history strip. Strip-stored cycles pull whole rows, so a cut is not per plot but once per tile, in the tile service's refine pass after the place is playable (`fetch_tile_lv.add_history`: the four cycles side by side, each JPEG at its sheets' own resolution (from the `.tfw`: 1 m to 0.25 m) beside `ortho.jpg`, listed as `history` in `terrain_meta.json`; 68 s for Rīga and Valka together). The 1st cycle is not in the bucket (403); Rīga has no 3rd-cycle sheet |
