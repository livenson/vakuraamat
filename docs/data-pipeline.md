...
| The cadastre's water-land parcels (Latvia) | open water as the cadastre draws it: the orthophoto repainted with the water's own tone over it, raised blobs over 5 m across (moored ships the laser classed as ground) lowered to its level, the canopy cleared; the laser's bridge decks kept out (a stream in its valley is left alone) | `water_parcels.paint`, right after the cadastre (`Latvia.registers`) | the tile's `ortho.jpg`, `heightmap.r32` and `canopy.r32` in place, noted as `water` in `terrain_meta.json` |
...
| OpenStreetMap `highway=bus_stop` nodes from the tile's one answer (`osm_tile.py`, ODbL) | bus stops, snapped to the nearest road with a heading | `tools/pipeline/fetch_stops.py` | `sites/<id>/stops.json` (shelters by `RoadNetwork._bus_stops`) |
...
| OpenStreetMap, one answer per tile (ODbL; every country): `osm_tile.elements` cuts the tile out of the country's Geofabrik extract with osmium-tool (`osm_extract.py`, about a second), else asks Overpass with one union query; either answer is cached for 7 days in `data_raw/overpass`, one request at a time, a stale answer when the servers fail | street lamps, traffic signals, crossings, benches, bins, bike racks, bollards, fences, hedges and walls, flowerbeds, piers; parking bays, with a parked car where the orthophoto shows one; tram and railway tracks with their stops; shops, cafés and offices with name, brand, opening hours and the company on their building; single trees with a height from the canopy model | `tools/pipeline/fetch_osm.py` (a tile-service stage after the registers) | `sites/<id>/street.json` (`StreetFurniture`, `TrafficSignals`, the lamps of `RoadNetwork`), `rail.json` (`Rails`, `RailVehicle`), `pois.json` (`Pois`: the shop signs, their hours, the building sheet), `assets/terrain/<tile>/trees_osm.json` (placed by the scatter where the tile has no `trees.json`) |
| Poly Haven road surfaces (CC0) | asphalt, setts, paving, cobbles, concrete, gravel, drawn by the surface a road's source names | `tools/pipeline/fetch_polyhaven.py --set roads` | `assets/textures/roads/` (`RoadNetwork.SURFACES`) |
| OpenStreetMap `way[highway]` from the tile's one answer (Latvia and Finland) | a tile's streets, paths and trails, split at every node two ways share so the road graph can turn there | `tools/pipeline/fetch_roads_lv.py` | `sites/<id>/roads.json` |
| Geofabrik country extracts (`download.geofabrik.de/europe/<country>-latest.osm.pbf`, ODbL; Estonia 123 MB, Latvia 140 MB, Finland 766 MB) | the OpenStreetMap of a whole country, cut per tile; downloaded when first needed and again after 7 days in the background, the old one serving up to 30 days | `tools/pipeline/osm_extract.py` (needs `osmium`, `brew install osmium-tool`; the frozen sidecar goes to Overpass instead) | `data_raw/osm/`, never shipped |
| ATD and Rīgas satiksme GTFS (Latvia, CC0) | the buses and trolleybuses calling at a Latvian tile's stops | `tools/pipeline/fetch_departures.py` (`fetch_lv`, chosen by the pack's `country`) | `sites/<id>/departures.json` |
| NLS 2 m ground model, 0.5 p laser points and orthophotos (the Funet mirror of the NLS open data, key-free; Finland, CC BY 4.0); inside Helsinki the city's 1 m ground model (WCS) and 5 cm photograph (WMS) | a Finnish tile's ground (the city's 1 m where it covers the tile, else 2 m), canopy from the laser points' split pulses with the footprints masked out, and orthophoto, reprojected from ETRS-TM35FIN onto the L-EST97 grid; the sea at its level | `tools/pipeline/fetch_tile_fi.py` (through the `Finland` adapter; see [finland-plan.md](finland-plan.md)) | the tile files |
| older NLS orthophoto years; in Helsinki the city's `Ortoilmakuva_<year>` layers (six spread from 1932 to 2015) | a Finnish plot's square in each year, for the plot page's history strip | `fetch_tile_fi.add_history`, in the tile service's refine pass | `assets/terrain/<tile>/ortho_<year>.jpg` |
| NLS INSPIRE cadastre WFS (`cp:CadastralParcel`); Helsinki's plot units (`avoindata:Kaavayksikot`); Ryhti's buildings and addresses (SYKE OGC API); footprints from Helsinki (`Rakennukset_alue_rekisteritiedot`), else the NLS topographic database's building layer or OpenStreetMap (Finland) | parcels (no land value exists; in Helsinki the zoning class and building rights), buildings (use, year, floors, material, the permanent building id) on their footprints with roofs fitted to the laser points | `tools/pipeline/fetch_cadastre_fi.py`, `roof_fit.py` | `sites/<id>/parcels.json`, `buildings.json` |
| PRH open data (YTJ daily bulk) and the Tax Administration's public corporate income tax file (Finland, CC BY 4.0) | companies at the tile's addresses with their industry, and the tax charged and taxable income for the newest tax year (no staff or turnover in open data) | `tools/pipeline/fetch_tenants_fi.py` | `sites/<id>/tenants.json` |
| HSL GTFS (Finland, CC BY 4.0) | the buses calling at a Helsinki-region tile's stops | `tools/pipeline/fetch_departures.py` (chosen by the pack's `country`) | `sites/<id>/departures.json` |
| Ruokavirasto INSPIRE WFS, agricultural parcels with the crop code and name (Finland, CC BY 4.0) | farmed fields and their declared crops | `tools/pipeline/fetch_fields_fi.py` | `sites/<id>/fields_2026.json` |
| Ryhti's addresses (Finnish and Swedish) and Statistics Finland's municipalities (`kunta4500k`) | the Locations search for a Finnish address; the outline of Finland for the menu's map and the adapter's coverage | `tools/pipeline/geocode_fi.py`, `fetch_outline.py` | `data_raw/fi/` (the index), `assets/data/finland.json` |
| the other country's registers, for a tile on the Estonian-Latvian border | the parcels, buildings, companies and (for an Estonian pack) OpenStreetMap roads on the other side, merged into the pack | `tools/pipeline/cross_border.py` (every tile-service job) | the pack's own files |
...
| `make stops SITE=<id>` | `sites/<id>/stops.json` | the tile's OpenStreetMap answer (`osm_tile.py`), the pack's `roads.json` |
| `make osm SITE=<id>` | `sites/<id>/street.json`, `rail.json`, `pois.json`, `assets/terrain/<tile>/trees_osm.json` | the tile's OpenStreetMap answer, the pack's buildings and tenants, the orthophoto (parked cars) and canopy (tree heights) |
| `make pitches SITE=<id>` | `sites/<id>/pitches.json` | the tile's OpenStreetMap answer |
