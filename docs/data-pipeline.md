...
| OpenStreetMap via Overpass, one query per tile (ODbL; every country) | street lamps, traffic signals, crossings, benches, bins, bike racks, bollards, fences, hedges and walls, flowerbeds, piers; parking bays, with a parked car where the orthophoto shows one; tram and railway tracks with their stops; shops, cafés and offices with name, brand, opening hours and the company on their building; single trees with a height from the canopy model | `tools/pipeline/fetch_osm.py` (a tile-service stage after the registers) | `sites/<id>/street.json` (`StreetFurniture`, `TrafficSignals`, the lamps of `RoadNetwork`), `rail.json` (`Rails`, `RailVehicle`), `pois.json` (`Pois`: the shop signs, their hours, the building sheet), `assets/terrain/<tile>/trees_osm.json` (placed by the scatter where the tile has no `trees.json`) |
...
| OpenStreetMap via Overpass, `way[highway]` (Latvia) | a Latvian tile's streets, paths and trails, split at every node two ways share so the road graph can turn there | `tools/pipeline/fetch_roads_lv.py` | `sites/<id>/roads.json` |
