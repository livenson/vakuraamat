	...
	for root in [ROOT, USER_ROOT]:
		...
		for sub in d.get_directories():
			if FileAccess.file_exists(root + sub + "/site.json") and not _root_of.has(sub):
				if root == USER_ROOT and _is_historical(root + sub + "/site.json"):
					continue   # a pack from the three-era game (tag v0.9-historical): its scenes no longer load
				available.append(sub)
				_root_of[sub] = root
	available.sort()


## Packs generated by the historical game carry a story, objectives or an ending in their manifest.
static func _is_historical(manifest_path: String) -> bool:
	var m = JSON.parse_string(FileAccess.get_file_as_string(manifest_path))
	if typeof(m) != TYPE_DICTIONARY:
		return true
	return m.has("story") or m.has("ending") or m.has("objectives") or m.has("register_nudge")
