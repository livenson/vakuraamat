...
| Godot-SpacetimeDB-SDK (plaught-armor, maintained hard fork of flametime's SDK) | 2.8.0, commit 9ceeab4 2026-08-28 | `addons/SpacetimeDB` (pure GDScript client: BSATN over WebSocket, auto-reconnect, headless codegen `cli.gd` writing `spacetime_bindings/`) | MIT (original flametime copyright retained) | keep `addons/SpacetimeDB/LICENSE`; generated `spacetime_bindings/` are ours | yes |
| SpacetimeDB (Clockwork Labs): server, CLI and the `spacetimedb` Rust crate used by `server/vakuraamat` | 2.10.0, 2026-09-05 | server side only (not shipped in the game build); module source is ours | Business Source License 1.1 (one production instance; converts to AGPL-3.0 with linking exception on 2031-09-03) | run a single production instance or Maincloud; do not offer SpacetimeDB itself as a service; the game and module code stay MIT | yes (module MIT; server under BSL terms) |
...
| Maa-amet cadastre, valid cadastral units (WFS `kataster:ky_kehtiv` on gsavalik.envir.ee: tunnus, address, purpose, area, ownership, polygon, the 2022 regular valuation's taxation value `maks_hind`, EHAK settlement code `hkood`, `ads_oid`) | fetched 2026-09-05 | `sites/<id>/parcels.json` (`tools/pipeline/fetch_parcels.py`), `sites/<id>/market.json` (`tools/pipeline/market.py`, medians derived from `maks_hind`), K overlay, parcel kits, the ledger | Maa-amet open data licence (attribution) | credit "Maakataster, Maa- ja Ruumiamet"; label the value as a 2022 taxation value, not a market price | yes (attribution) |
| e-Business Register open data, basic company data (`avaandmed.ariregister.rik.ee`, `ettevotja_rekvisiidid__lihtandmed.csv.zip`: name, registry code, legal form, status, first registration date, address, EHAK code) | daily file, fetched 2026-09-05, cached in `data_raw/ariregister/` | `sites/<id>/tenants.json` (`tools/pipeline/fetch_tenants.py`), tenant names in the K overlay and the ledger | CC BY 4.0 | credit "Äriregistri avaandmed, Registrite ja Infosüsteemide Keskus"; legal persons only (sole proprietors skipped), VAT numbers, postcodes and ADS ids not stored | yes (attribution) |
| Ametlikud Teadaanded (Official Announcements) XML search, types `planeerimismenetlus` and `enampakkumine` only | live, `tools/news_feeder.py` | town `event` rows / `sites/<id>/news.json` (not in git) | public notices under the Official Announcements Act; body text has reuse limits after archiving | store only the notice type, a composed title with the address, date and link; never publisher, addressee or body; no person-directed notice types | yes (metadata only) |
| ERR news RSS (`www.err.ee/rss`, items tagged Eesti) | live, `tools/news_feeder.py` | same | ERR: RSS feeds are free to embed (err.ee terms) | headline, date and link only; source named | yes |
| Postimees regional RSS (`tartu.postimees.ee/rss`, `lounapostimees.postimees.ee/rss`) | live, `tools/news_feeder.py` | same | publisher terms (no formal open licence) | headlines and links only, no body text or images; source named; remove on request | yes (headlines and links) |
...
| Kenney Furniture Kit (Kenney, www.kenney.nl): beds, tables, chairs, sofas, shelves, desks, kitchen units, plants, lamps, boxes (24 of 140 models) | 2.0, 2018-10-20, fetched 2026-09-05 | `assets/vendor/kenney_furniture_kit/glb` (licence file alongside), interiors of real buildings | CC0 1.0 | none | yes |
| Poly Pizza models, Kenney (kenney.nl): toilet, bathtub, bathroom sink, bathroom mirror | fetched 2026-09-06 | `assets/vendor/polypizza/{toilet,bathtub,sink,mirror}.glb`, bathrooms in the interiors | CC0 1.0 | none | yes |
...
| Cogito, immersive-sim template (Philip Drobar, codeberg.org/Phazorknight/Cogito): the door swing, switch, carryable and readable interaction patterns, ported and reduced (no code vendored) | v1.1.6, 2026-05; read 2026-09-06 | `scripts/interaction/swing_door.gd`, `light_switch.gd`, `carryable.gd`, `readable.gd` | MIT | keep the credit in the file headers and here | yes |
